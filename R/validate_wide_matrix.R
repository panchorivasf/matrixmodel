...
#'
#' @param df A data frame to validate. Expected to contain a 'PlotID' column and
#'   matrix columns with names following the pattern 'S##_D##' (e.g., S01_D01,
#'   S02_D15), where ## represents two-digit numbers.
#'
#' @returns A character vector of error messages. If the data frame is valid,
...
validate_wide_matrix <- function(df) {
  errs <- c()
  if (!"PlotID" %in% names(df)) errs <- c(errs, "Missing required column: PlotID")
  mat_cols <- grep("^S[0-9]{2}_D[0-9]{2}$", names(df), value = TRUE)
  if (length(mat_cols) == 0) errs <- c(errs, "No matrix columns found. Expect names like S01_D01, S01_D02, ...")
  if (length(errs)) return(errs)
  non_num <- mat_cols[!sapply(df[mat_cols], is.numeric)]
  if (length(non_num)) errs <- c(errs, paste0("Non-numeric matrix columns: ", paste(non_num, collapse=", ")))
  errs
